...
namespace PathFindWrappers
{
    ...
    {
        TheNewPaths.AllocatePathFindInfoMem(numPathGroups);
    }
    static void __fastcall RegisterMapObject(void* ECX, void* EDX, CTreadableRe* mapObject)
    {
        TheNewPaths.RegisterMapObject(mapObject);
    }
    ...
    {
        TheNewPaths.StoreNodeInfoPed(id, node, type, next, x, y, z, width, crossing);
    }
#ifndef FIX_BUGS
    ...
    {
        TheNewPaths.PreparePathData();
    }
    static float __fastcall CalcRoadDensity(void* ECX, void* EDX, float x, float y)
    {
        return TheNewPaths.CalcRoadDensity(x, y);
    }
    static bool __fastcall TestForPedTrafficLight(void* ECX, void* EDX, CPathNodeRe* n1, CPathNodeRe* n2)
    {
        return TheNewPaths.TestForPedTrafficLight(n1, n2);
    }
    static bool __fastcall TestCrossesRoad(void* ECX, void* EDX, CPathNodeRe* n1, CPathNodeRe* n2)
    {
        return TheNewPaths.TestCrossesRoad(n1, n2);
    }
    static void __fastcall RemoveBadStartNode(void* ECX, void* EDX, CVector pos, CPathNodeRe** nodes, int16* n)
    {
        TheNewPaths.RemoveBadStartNode(pos, nodes, n);
    }
    static void __fastcall SetLinksBridgeLights(void* ECX, void* EDX, float x1, float x2, float y1, float y2, bool enable)
    {
        TheNewPaths.SetLinksBridgeLights(x1, x2, y1, y2, enable);
    }
    static void __fastcall SwitchOffNodeAndNeighbours(void* ECX, void* EDX, int32 nodeId, bool disable)
    {
        TheNewPaths.SwitchOffNodeAndNeighbours(nodeId, disable);
    }
    ...
    {
        TheNewPaths.SwitchRoadsOffInArea(x1, x2, y1, y2, z1, z2, disable);
    }
    ...
    {
        TheNewPaths.SwitchPedRoadsOffInArea(x1, x2, y1, y2, z1, z2, disable);
    }
    ...
    {
        TheNewPaths.SwitchRoadsInAngledArea(x1, x2, y1, y2, z1, z2, length, type, enable);
    }
    ...
    {
        TheNewPaths.PedMarkRoadsBetweenLevelsInArea(x1, x2, y1, y2, z1, z2);
    }
    ...
    {
        return TheNewPaths.FindNodeClosestToCoors(coors, type, distLimit, ignoreDisabled, ignoreBetweenLevels);
    }
    ...
    {
        return TheNewPaths.FindNodeClosestToCoorsFavourDirection(coors, type, dirX, dirY);
    }
    static float __fastcall FindNodeOrientationForCarPlacement(void* ECX, void* EDX, int32 nodeId)
    {
        return TheNewPaths.FindNodeOrientationForCarPlacement(nodeId);
    }
    ...
    {
        return TheNewPaths.FindNodeOrientationForCarPlacementFacingDestination(nodeId, x, y, towards);
    }
    ...
    {
        return TheNewPaths.NewGenerateCarCreationCoors(x, y, dirX, dirY, spawnDist, angleLimit, forward, pPosition, pNode1, pNode2, pPositionBetweenNodes, ignoreDisabled);
    }
    ...
    {
        return TheNewPaths.GeneratePedCreationCoors(x, y, minDist, maxDist, minDistOffScreen, maxDistOffScreen, pPosition, pNode1, pNode2, pPositionBetweenNodes, camMatrix);
    }
    static void __fastcall FindNextNodeWandering(void* ECX, void* EDX, uint8 type, CVector coors, CPathNodeRe** lastNode, CPathNodeRe** nextNode, uint8 curDir, uint8* nextDir)
    {
        TheNewPaths.FindNextNodeWandering(type, coors, lastNode, nextNode, curDir, nextDir);
    }
    static void __fastcall DoPathSearch(void* ECX, void* EDX, uint8 type, CVector start, int32 startNodeId, CVector target, CPathNodeRe** nodes, int16* numNodes, int16 maxNumNodes, CPhysical* vehicle, float* dist, float distLimit, int32 forcedTargetNode)
    {
        TheNewPaths.DoPathSearch(type, start, startNodeId, target, nodes, numNodes, maxNumNodes, vehicle, dist, distLimit, forcedTargetNode);
    }
    static bool __fastcall TestCoorsCloseness(void* ECX, void* EDX, CVector target, uint8 type, CVector start)
    {
        return TheNewPaths.TestCoorsCloseness(target, type, start);
    }
    static void __fastcall Save(void* ECX, void* EDX, uint8* buf, uint32* size)
    {
        TheNewPaths.Save(buf, size);
    }
    static void __fastcall Load(void* ECX, void* EDX, uint8* buf, uint32 size)
    {
        TheNewPaths.Load(buf, size);
    }
}
